...
// 1. Create Project with image (uses FormData)
// 1. Create Project with image (uses FormData)
export const createProject = createAsyncThunk(
  ...
  async (newProjectData, { rejectWithValue }) => {
    try {
      ...
      // ⛔️ DO NOT rewrap newProjectData as FormData again
      const config = {
        headers: {
          Authorization: `Bearer ${token}`, // Let browser handle multipart
        },
      };

      const response = await axios.post(
        `${base_url}/projects`,
        newProjectData, // 👈 This is already a FormData instance
        config
      );
...
// 4. Update Project with optional image
