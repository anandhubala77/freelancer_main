...
import {
  FaUsers, FaFolder, FaShieldAlt, FaUserTie, FaUser, FaUserCircle,
...
const Dashboard = () => {
  ...
  const { metrics, loading, error } = useSelector((state) => state.adminDashboard);
  ...
  const metricsData = [
    { title: 'Total Users', value: metrics?.totalUsers ?? 0, icon: FaUsers, color: 'bg-indigo-600' },
    { title: 'Total Projects', value: metrics?.totalProjects ?? 0, icon: FaFolder, color: 'bg-green-600' },
    { title: 'Fraud Reports', value: metrics?.fraudReports ?? 0, icon: FaShieldAlt, color: 'bg-red-600' },
    { title: 'Job Seekers', value: metrics?.jobSeekers ?? 0, icon: FaUser, color: 'bg-purple-600' },
    { title: 'Hiring Persons', value: metrics?.hiringPersons ?? 0, icon: FaUserTie, color: 'bg-yellow-500' },
  ];

  return (
    <div ref={contentRef} className="w-full px-2 py-4 sm:px-4 md:px-6 lg:px-8 overflow-y-auto min-h-screen bg-gray-50">
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-lg shadow p-4">
              <h2 className="text-lg font-bold mb-4 text-gray-700">Recent Projects</h2>
              <div className="flex flex-col gap-3 max-h-[350px] overflow-y-auto">
                {metrics?.recentProjects?.length > 0 ? (
                  metrics.recentProjects.map((project, index) => (
                    <div key={index} className="flex items-center gap-4 p-3 rounded-lg border hover:shadow transition bg-gray-50">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-2 text-xs mt-1">
                          <span className="flex items-center gap-1 text-gray-600">
                            <FaMoneyBillWave className="inline-block" /> ₹{project.budget}
                          </span>
                          <span className={`px-2 py-0.5 rounded-full font-medium ${statusColors[project.status] || "bg-gray-200 text-gray-700"}`}>
                            {project.status}
                          </span>
                        </div>
                      </div>
            ...
            {/* Recent Users */}
            <div className="bg-white rounded-lg shadow p-4">
              <h2 className="text-lg font-bold mb-4 text-gray-700">Recent Users</h2>
              <div className="flex flex-col gap-3 max-h-[350px] overflow-y-auto">
                {metrics?.recentUsers?.length > 0 ? (
                  metrics.recentUsers.map((user, index) => (
                    <div key={index} className="flex items-center gap-4 p-3 rounded-lg border hover:shadow transition bg-gray-50">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-gray-800 truncate">{user.name} {user.lastName}</p>
          ...
          <div className="bg-white rounded-lg shadow p-4 mt-8">
            <h2 className="text-lg font-bold mb-4 text-gray-700">Recent Bids</h2>
            <div className="flex flex-col gap-3 max-h-[350px] overflow-y-auto">
              {metrics?.recentBids?.length > 0 ? (
                metrics.recentBids.map((bid, index) => (
                  <div key={index} className="flex items-center gap-4 p-3 rounded-lg border hover:shadow transition bg-gray-50">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-gray-800 truncate">{bid.jobId?.title || "Unknown Project"}</p>
          ...
          {/* Recent Payments */}
          <div className="bg-white rounded-lg shadow p-4 mt-8">
            <h2 className="text-lg font-bold mb-4 text-gray-700">Recent Payments</h2>
            <div className="flex flex-col gap-3 max-h-[350px] overflow-y-auto">
              {metrics?.recentPayments?.length > 0 ? (
                metrics.recentPayments.map((payment, index) => (
                  <div key={index} className="flex items-center gap-4 p-3 rounded-lg border hover:shadow transition bg-gray-50">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-gray-800 truncate">{payment.jobId?.title || "Unknown Project"}</p>
                      <div className="text-xs text-gray-600 mt-1">
                        ₹{payment.amount} paid by {payment.paidBy?.name || "Unknown"} to {payment.paidTo?.name || "Unknown"}
                      </div>
                    </div>
                  </div>
                ))
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-10">
            ...
            <div className="bg-white rounded-lg shadow p-4">
              <h2 className="text-lg font-bold text-gray-700 mb-4">Monthly Payments</h2>
            ...
            <div className="bg-white rounded-lg shadow p-4">
              <h2 className="text-lg font-bold text-gray-700 mb-4">Yearly Payments</h2>
              <ResponsiveContainer width="100%" height={250}>
